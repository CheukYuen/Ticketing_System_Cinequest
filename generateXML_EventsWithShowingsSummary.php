<?php 
	...
	//      SHOWING ticketing details: Available, Sold, Disabled, InProcess
	// --------------------------------------------------------------------- //
	...
	while ($row = mysql_fetch_assoc($result)) {
		...
		while ($rowShowing = mysql_fetch_assoc($resultShowing)) {
			...
			// Save the SHOWING and ticketing details in Showing array
			...
			$Available = $rowShowing['Available'];
			$Disabled = $rowShowing['Disabled'];
			$Sold = $rowShowing['Sold'];
			$InProcess = $rowShowing['InProcess'];
	...
	$phpResponseToXML->asXML('XMLFiles/EventsWithShowingSummary.xml');
